...
{
	out8(irqmask & 0xFF, PIC8259_IO_PIC1 + 1);
}

static void pic_slave_set_mask(void)
{
	out8(irqmask >> 8, PIC8259_IO_PIC2 + 1);
}
...
void pic8259_init(void)
{
	...
	out8(0xFF, PIC8259_IO_PIC1 + 1);
	out8(0xFF, PIC8259_IO_PIC2 + 1);

	// restart PIC
	out8(0x11, PIC8259_IO_PIC1);
	out8(0x11, PIC8259_IO_PIC2);

	// PIC1 now starts at PIC8259_IRQ0
	out8(PIC8259_IRQ0, PIC8259_IO_PIC1 + 1);
	// PIC2 now starts at PIC8259_IRQ8
	out8(PIC8259_IRQ0 + 8, PIC8259_IO_PIC2 + 1);

	// setup cascading
	out8(1 << PIC8259_IRQ_SLAVE, PIC8259_IO_PIC1 + 1);
	out8(PIC8259_IRQ_SLAVE, PIC8259_IO_PIC2 + 1);

	out8(0x01, PIC8259_IO_PIC1 + 1);
	out8(0x01, PIC8259_IO_PIC2 + 1); // done!
...
void pic8259_timer_init(void)
{
	...
	out8(PIC8259_TIMER_MODE_REG,
		PIC8259_TIMER_SEL0 |
		PIC8259_TIMER_RATGEN |
		PIC8259_TIMER_16BIT
	);
	out8(PIC8259_TIMER_DIV(TIMER_TICKS_PER_SECOND) % 256, PIC8259_TIMER_IOBASE);
	out8(PIC8259_TIMER_DIV(TIMER_TICKS_PER_SECOND) / 256, PIC8259_TIMER_IOBASE);
...
{
	out8(PIC8259_READ_ISR, PIC8259_IO_PIC1);
	return in8(PIC8259_IO_PIC1);
}

static uint8_t
pic8259_read_slave_isr(void)
{
	out8(PIC8259_READ_ISR, PIC8259_IO_PIC2);
	return in8(PIC8259_IO_PIC2);
}
...
void pic8259_send_eoi(int irq)
{
	...
	if (irq >= 8) {
		...
		if (isr & bit) {
			out8(PIC8259_EOI, PIC8259_IO_PIC2);
		}

		out8(PIC8259_EOI, PIC8259_IO_PIC1);
	} else {
